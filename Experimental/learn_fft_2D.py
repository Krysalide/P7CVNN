...
X = np.concatenate(
    [sig.real.reshape(batch, -1), sig.imag.reshape(batch, -1)], axis=1
).astype(np.float32)
Y = np.concatenate(
    [F.real.reshape(batch, -1), F.imag.reshape(batch, -1)], axis=1
).astype(np.float32)
...
def ANN_2DFFT(x_np):
    ...
    x_input = np.concatenate([x_np.real.flatten(), x_np.imag.flatten()]).astype(
        np.float32
    )
