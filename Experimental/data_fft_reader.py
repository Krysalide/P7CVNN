...
class RadarDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = np.load(
            os.path.join(self.save_folder, f"raw_adc_{i}.npy")
        )  # shape: (512, 256, 16), dtype: complex64
        y = np.load(
            os.path.join(self.save_folder, f"range_doppler_map_{i}.npy")
        )  # shape: ?, dtype: complex64 or float
...
class RadarDatasetV2(Dataset):
    def __init__(
        ...
        if recursive:
            ...
            self.x_paths = sorted(
                glob(os.path.join(save_folder, "**", "raw_adc_*.npy"), recursive=True)
            )
            self.y_paths = [
                p.replace("raw_adc_", "range_doppler_map_") for p in self.x_paths
            ]
        else:
            ...
            self.x_paths = [
                os.path.join(save_folder, f"raw_adc_{i}.npy") for i in indices
            ]
            self.y_paths = [
                os.path.join(save_folder, f"range_doppler_map_{i}.npy") for i in indices
            ]
