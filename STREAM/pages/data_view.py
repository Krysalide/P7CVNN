...
element_number = 60
assert element_number <= 60, (
    f"Number of elements is limited to 60. You requested {element_number}."
)
indices = list(range(element_number))  # specify number of elements

dataset = RadarFFTDataset(data_folder, indices)
st.write(
    f"Dataset loaded with {len(dataset)} samples (showing first {element_number})."
)
...
if (
    raw_adc_test.shape[-1] <= selected_antenna_idx
    or range_doppler_test.shape[-1] <= selected_antenna_idx
):
    st.error(
        f"Error: Selected antenna {antenna} is out of bounds for the loaded data. "
